"""Audio-capacity audit against a LIVE endpoint (MMSU edition of the run18/19 tool).
...
This is a TOKEN-FIT gate, not a clip-length gate — MMSU's longest clip is ~35.9 s
(well within Qwen2.5-Omni's 32k window). Do NOT trust a first-N smoke instead of
this — metadata order biases short audio (SETUP_GUIDE §10 anomaly 4).
"""
...
def main(endpoint, model_name, data_root, subset, audio_root, n_longest, n_rate,
    ...
    records = load_mmsu_mcq(data_root, subset=subset, audio_root=audio_root)
