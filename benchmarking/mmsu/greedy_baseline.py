"""Greedy (temp 0), NO-CoT, single-shot direct-answer baseline over the full MMSU set.
...
numbers from the particle runs.
...
directly comparable. Concurrent across endpoints, resumable via the CSV.
    ...
    python -m benchmarking.mmsu.greedy_baseline \
        --endpoints http://localhost:8100/v1,http://localhost:8101/v1 --model-name qwen-omni \
        --csv  benchmarking/mmsu/results/run00_greedy_nocot/greedy.csv \
        --log  benchmarking/mmsu/results/run00_greedy_nocot/greedy.log
"""
...
def main(endpoints, model_name, api_key, data_root, subset, audio_root, audio_mode,
    ...
    records = load_mmsu_mcq(data_root, subset=subset, limit=limit, audio_root=audio_root)
    ...
    coarse = {u: v["category"] for u, v in load_metadata(data_root, subset=subset).items()}
    ...
    for r in rows:
        if r["correct"] not in ("True", "False"):
            continue
        c = coarse.get(r["unique_id"], "?")
        a = split.setdefault(c, [0, 0])
        a[0] += r["correct"] == "True"
        a[1] += 1
    ...
    for c in sorted(split):
        s, m = split[c]
        lines.append(f"  {c:12s}     : {s}/{m} = {s / max(m,1):.4f}")
    report = "\n".join(lines)
    print("\n" + report)
    if log_path:
