...
# configs may override (e.g. the Mellow shim serves one request at a time per GPU and
# prefers 32; EXTRA_PROBE_ARGS carries per-model probe flags like Mellow's --stop-regex)
MAX_INFLIGHT="${MAX_INFLIGHT:-64}"
MAX_INFLIGHT_B1="${MAX_INFLIGHT_B1:-24}"
EXTRA_PROBE_ARGS="${EXTRA_PROBE_ARGS:-}"

for B in $BUDGETS; do
    INFLIGHT=$MAX_INFLIGHT
    [ "$B" -eq 1 ] && INFLIGHT=$MAX_INFLIGHT_B1
    ok=0
