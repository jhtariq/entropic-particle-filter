#!/usr/bin/env bash
# run_grid.sh <config_runNN.sh> — budget-staged EPF grid over the full MMAR set,
# resumable JSONL, up to 3 attempts per stage. A stage passes only if (a) no error
# rows remain (summarize_errors) AND (b) every cell at this budget has all 1,000
...
#   nohup setsid bash benchmarking/mmar/scripts/run_grid.sh \
#     benchmarking/mmar/scripts/config_run01.sh \
...
SUBSET="${SUBSET:-full}"          # configs may override (e.g. "le30s" for Qwen2-Audio)
N_ITEMS="${N_ITEMS:-1000}"        # items in $SUBSET (drives the per-stage count gate)
EXPECTED=$(( $(echo "$PROMPTS" | tr ',' '\n' | grep -c .) * 2 * N_ITEMS ))  # prompts x signals x items
mkdir -p "$OUT_DIR"
cd "$REPO_ROOT"
...
for B in $BUDGETS; do
    ...
    for attempt in 1 2 3; do
        ...
        "$EPF_PY" -m benchmarking.mmar.diversity_probe \
            --endpoints "$ENDPOINTS" --model-name "$SERVED_NAME" \
            --data-root "$DATA_ROOT" --subset "$SUBSET" \
            --prompts "$PROMPTS" --signals mean_logprob,entropy --budgets "$B" \
        ...
        if ! "$EPF_PY" -m benchmarking.mmar.summarize_errors "$J"; then
