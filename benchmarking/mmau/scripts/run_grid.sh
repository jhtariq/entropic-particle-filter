#!/usr/bin/env bash
# run_grid.sh <config_runNN.sh> — budget-staged EPF grid over the full MMAU
# test-mini set, resumable JSONL, up to 3 attempts per stage. A stage passes only
# if (a) no error rows remain (summarize_errors) AND (b) every cell at this budget
# has all 1,000 clean rows — so a probe process dying without recording errors
# cannot silently pass a stage. b1 runs --max-inflight 24 (audio-encoder OOM
# lesson, SETUP_GUIDE §10.1), else 64.
# Run detached (literal example for Run 1):
#   nohup setsid bash benchmarking/mmau/scripts/run_grid.sh \
#     benchmarking/mmau/scripts/config_run01.sh \
#     > benchmarking/mmau/results/run01_omni7b/stages.log 2>&1 &
set -u
...
N_ITEMS="${N_ITEMS:-1000}"        # items in $SUBSET (drives the per-stage count gate)
...
for B in $BUDGETS; do
    ...
    for attempt in 1 2 3; do
        ...
        "$EPF_PY" -m benchmarking.mmau.diversity_probe \
            ...
            --select all --limit 1100 --max-inflight "$INFLIGHT" \
            --jsonl "$J" --csv "$C" --log "$L"
        probe_rc=$?
        ...
        if ! "$EPF_PY" -m benchmarking.mmau.summarize_errors "$J"; then
