"""Audio-capacity audit against a LIVE endpoint (Run 18 / Phi-4-MM tool).

vLLM audio models expand clips into prompt tokens SERVER-side (Phi-4-MM: conformer
frames / compression_rate), so offline token estimates are unreliable — run17's
audit_prompt_tokens.py (offline SmolLM2 counts) does not transfer. This tool asks
the served model instead:
...
Do NOT trust a first-N smoke instead of this — parquet order biases short audio
(SETUP_GUIDE §10 anomaly 4).
"""
...
@click.option("--model-name", required=True, help="lora mode: the ADAPTER name (lib.sh probe_model_name)")
@click.option("--data-root", default="/home/exx/inference-time-scaling/mmau_pro_testmini")
@click.option("--subset", default="test")
