...
# Idempotent — safe to re-run after an interrupted download. Same pinned sources
# as run16; only step 2 (the committed Run-17 subset parquets) differs.
set -euo pipefail
...
echo "== 2/5 derived Run-17 subset parquets (committed in run17/data/)"
cp -n "$RUN17_DIR/data/test_no3a-00000-of-00001.parquet"       "$DATA_TESTMINI/data/" || true
cp -n "$RUN17_DIR/data/test_le10s_flat-00000-of-00001.parquet" "$DATA_TESTMINI/data/" || true
...
"$EPF_PY" "$RUN17_DIR/prep_testmini_audio.py" \
...
n3a_ungr = sum(1 for r in n3a if r.answer_index is None)
assert (len(n3a), n3a_ungr) == (5073, 24), f"test_no3a: got {len(n3a)}/{n3a_ungr}, want 5073/24"
assert len(load_mmau_mcq(tm, subset="test_le10s_flat", audio_root=au)) == 326
print("DATA OK: test=5090 MCQ (24 ungradeable), full=957, test_no3a=5073, test_le10s_flat=326")
PYEOF
