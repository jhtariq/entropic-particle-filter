...
exec 9> "$OUT_ROOT/.run.lock${LOCK_SUFFIX:-}"
if ! flock -n 9; then
...
for MODEL in $B_MODELS; do
  ...
  if [ "$M_SERVE" = "mellow" ]; then
    got=$(git -C "$MELLOW_CODE_DIR" rev-parse HEAD 2>/dev/null || echo none)
    [ "$got" = "$MELLOW_CODE_COMMIT" ] || { echo "  MISSING/WRONG mellow code clone -> $MELLOW_CODE_DIR @ $got" >&2; miss=1; }
    SM="$HF_HOME/hub/models--${SMOLLM2_ID//\//--}/snapshots/$SMOLLM2_REV"
    [ -d "$SM" ] && [ -n "$(ls -A "$SM" 2>/dev/null)" ] || { echo "  MISSING SmolLM2 base -> $SM" >&2; miss=1; }
  fi
done
...
for MODEL in $B_MODELS; do
  ...
  if [ "$M_SERVE" = "mellow" ]; then
    # the shim has no KV budget to plan: one batched request at a time per process,
    # so a whole n=128 chunk is one request and the per-model inflight cap is the lever
    KVP_MAXSEQS=0; KVP_NCHUNK="$SC_N"; KVP_INFLIGHT="$M_MAX_INFLIGHT"
    echo "  mellow shim: n_chunk=$KVP_NCHUNK inflight=$KVP_INFLIGHT (kv_plan bypassed)"
  else
    kv_plan "$M_KV_KIB" "$M_WEIGHTS_GIB" "$COT_MAX_TOKENS" "$SC_N"
    # tiny-KV models (gemma4: 35 KiB/tok) would get absurd seq counts; clamp for sane startup
    [ "$KVP_MAXSEQS" -gt 512 ] && KVP_MAXSEQS=512
    echo "  kv_plan: $KVP_NOTE -> max_num_seqs=$KVP_MAXSEQS n_chunk=$KVP_NCHUNK inflight=$KVP_INFLIGHT"
  fi

  if [ "$DRY_RUN" = "1" ]; then
    for ARM in ${M_ARMS:-$MMAR_ARMS}; do arm_cfg "$ARM"
  ...
  for ARM in ${M_ARMS:-$MMAR_ARMS}; do
